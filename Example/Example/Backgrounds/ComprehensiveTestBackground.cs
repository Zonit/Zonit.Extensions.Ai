...
internal class ComprehensiveTestBackground(IAiProvider provider) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        var tests = new (string Name, Func<CancellationToken, Task> Test)[]
        {
            ...
            ("Anthropic - Enum/Complex Types", async ct => await TestEnumStructuredOutput(new Sonnet46(), "Anthropic", ct)),
    ...
    {
        // Sonnet 4.6 uses adaptive thinking — the legacy budget_tokens knob is
        // replaced by Reason (effort); the model manages the budget itself.
        var model = new Sonnet46
        {
            Reason = Sonnet46.ReasonType.Medium
        };
    ...
    private async Task TestAllValueTypesAnthropic(CancellationToken ct)
    {
        try
        {
            await TestAllValueTypes(new Sonnet46(), "Anthropic", ct);
        }
