...
{
    private readonly IAnthropicTransport _transport;
    private readonly ILogger<AnthropicProvider> _logger;

    // Takes IServiceProvider (public) rather than IAnthropicTransport (internal)
    // directly: AnthropicProvider is public, so a public constructor may not expose a
    // less-accessible parameter type (CS0051). The active transport — API or CLI,
    // per AnthropicOptions.Transport — is resolved from the container here.
    public AnthropicProvider(
        IServiceProvider serviceProvider,
        ILogger<AnthropicProvider> logger)
    {
        _transport = serviceProvider.GetRequiredService<IAnthropicTransport>();
        _logger = logger;
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var anthropicResponse = await _transport.SendAsync(llm, request, "GenerateAsync", cancellationToken);
    ...
    /// </summary>
    internal static Exception BuildEmptyResponseError(ILogger logger, string operation, ILlm llm, string? stopReason, string? requestId)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var anthropicResponse = await _transport.SendAsync(llm, request, "ChatAsync", cancellationToken);
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var delta in _transport.StreamAsync(llm, request, "StreamAsync", cancellationToken))
            yield return delta;
    }
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var delta in _transport.StreamAsync(llm, request, "ChatStreamAsync", cancellationToken))
            yield return delta;
    }
    ...
    internal const string StructuredToolName = "respond_json";
