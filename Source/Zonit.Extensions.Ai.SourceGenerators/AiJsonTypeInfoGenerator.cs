...
public class AiJsonTypeInfoGenerator : IIncrementalGenerator
{
    ...
    private static void EmitPocoFactory(StringBuilder sb, PocoTypeInfo poco)
    {
        ...
        // For types with `required` members a plain `new T()` is a compile error (CS9035).
        // Bind the parameterless constructor via UnsafeAccessor instead: the raw newobj
        // bypasses the compile-time required-init check, and STJ then sets every property
        // and validates the IsRequired metadata. Fully supported under NativeAOT.
        if (hasAnyRequired)
        {
            sb.AppendLine($"    [global::System.Runtime.CompilerServices.UnsafeAccessor(global::System.Runtime.CompilerServices.UnsafeAccessorKind.Constructor)]");
            sb.AppendLine($"    private static extern {fq} __ctor_{poco.MangledName}();");
            sb.AppendLine();
        }
        ...
        // ObjectCreator constructs the instance; STJ then populates the properties and
        // validates required members via the IsRequired metadata below. Required-member
        // types route through the UnsafeAccessor constructor emitted above (a plain
        // `new T()` would trip CS9035); all others use a direct `new T()`.
        if (hasAnyRequired)
        {
            sb.AppendLine($"            ObjectCreator = static () => __ctor_{poco.MangledName}(),");
        }
        else
        {
    ...
    private sealed class TypeCollector
    {
        ...
        public void Visit(ITypeSymbol type)
        {
            ...
            if (type is INamedTypeSymbol poco
                ...
                && !poco.IsGenericType
                && SymbolAccessibility.IsAccessibleToAssembly(poco)
                && !poco.IsAbstract
        ...
        private static PocoTypeInfo? BuildPocoInfo(INamedTypeSymbol type)
        {
            ...
            // A struct with `required` members cannot be constructed through an
            // UnsafeAccessor parameterless ctor (value types expose no callable instance
            // .ctor to bind), so let those rare types fall back to reflection instead of
            // emitting broken metadata.
            if (type.TypeKind == TypeKind.Struct && props.Any(p => p.IsRequired))
                return null;
