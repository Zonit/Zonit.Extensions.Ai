...
public sealed class PerplexityProvider : IModelProvider
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogError("Perplexity error: {Status} - {Response}", response.StatusCode, responseJson);
        ...
        if (string.IsNullOrEmpty(textContent))
            throw new AiEmptyResponseException(AiResponseError.EmptyAfterRetries, "Perplexity returned no text — server-side data loss; usually transient, re-run the operation.");

        var result = ParseResponse<TResponse>(textContent);

        var inputTokens = perplexityResponse.Usage?.PromptTokens ?? 0;
        var outputTokens = perplexityResponse.Usage?.CompletionTokens ?? 0;
        ...
        return new Result<TResponse>
        {
            ...
            MetaData = new MetaData
            {
                ...
                RequestId = perplexityResponse.Id,
    ...
    public Task<Result<Asset>> GenerateImageAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        throw new NotSupportedException("Perplexity does not support image generation");
    }
    ...
    public Task<Result<Asset>> GenerateVideoAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        throw new NotSupportedException("Perplexity does not support video generation");
    }
    ...
    public Task<Result<string>> TranscribeAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        throw new NotSupportedException("Perplexity does not support audio transcription");
    }

    private void ConfigureHttpClient()
    {
        var baseUrl = _options.BaseUrl ?? "https://api.perplexity.ai";
...
internal sealed class PerplexityChoice
{
    public PerplexityMessage? Message { get; set; }
}

internal sealed class PerplexityMessage
{
    public string? Content { get; set; }
}

internal sealed class PerplexityUsage
{
    public int PromptTokens { get; set; }
    public int CompletionTokens { get; set; }
}

internal sealed class PerplexityStreamChunk
{
    public PerplexityStreamChoice[]? Choices { get; set; }
}

internal sealed class PerplexityStreamChoice
{
