...
internal sealed class OpenAiAgentSession : IAgentSession
{
    ...
    // enforced by IL2046/IL3051). The core AgentRunner's terminal step parses the final
    // TResponse and may invoke reflection-based tools. The request build here is AOT-safe:
    // schema via AiSchemaRegistry, serialization via the source-generated JsonContext.
    [RequiresUnreferencedCode("The agent run's terminal step parses the final TResponse and may invoke reflection-based tools.")]
    [RequiresDynamicCode("The agent run may perform reflection-based tool (de)serialization.")]
    ...
    /// <summary>
    /// Returns <c>true</c> when the JSON Schema root looks like a valid
    /// OpenAI strict-function-tool parameter object — i.e. an
    /// <c>{"type":"object", ..., "additionalProperties":false}</c> shape.
    /// We only check the root: emitting <c>additionalProperties:false</c>
    /// at the top is the explicit signal that the author authored the
    /// schema with strict mode in mind. Strict-mode validation on the
    /// server side then enforces the rest of the contract.
    /// </summary>
    private static bool HasStrictRoot(JsonElement schema)
    {
        if (schema.ValueKind != JsonValueKind.Object)
            return false;
        if (!schema.TryGetProperty("additionalProperties", out var ap))
            return false;
        return ap.ValueKind == JsonValueKind.False;
    }
}
