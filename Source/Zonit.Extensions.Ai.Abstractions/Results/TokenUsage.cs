...
{
    /// <summary>
    /// Total input/prompt tokens, INCLUSIVE of <see cref="CachedTokens"/> and
    /// <see cref="CacheWriteTokens"/> (the OpenAI usage convention).
    /// <c>AiCostCalculator</c> prices the uncached remainder as
    /// <c>InputTokens - CachedTokens - CacheWriteTokens</c>, so providers whose API
    /// reports input EXCLUSIVE of cache (e.g. Anthropic, where input_tokens,
    /// cache_read and cache_creation are disjoint) must add the cache buckets back
    /// in before setting this — otherwise the uncached input is billed at zero.
    /// </summary>
