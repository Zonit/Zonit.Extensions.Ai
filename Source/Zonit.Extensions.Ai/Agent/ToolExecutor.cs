...
internal sealed class ToolExecutor
{
    ...
    private readonly AiUsageScopeBuilder _agentScope;
    ...
    public ToolExecutor(
        ...
        AiUsageScopeBuilder agentScope,
        ILogger logger)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Open a grouping node for this tool call and make it current. Any AI call
        // the tool makes (it may inject IAiProvider) attaches as a child of this node,
        // so its tokens/cost/output are no longer invisible. The node is popped on
        // every exit path via the finally; the nested rollup is read into the
        // ToolInvocation for the common "how much AI did this tool use?" view.
        var toolScope = _tracker.BeginChild(
            _agentScope, AiUsageKind.Tool,
            toolName: call.Name, toolCallId: call.Id, toolIteration: iteration);
        ...
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            var nested = NestedOf(toolScope); // sub-calls may have completed before the timeout
            return (new ToolInvocation
            {
                Iteration = iteration,
                Name = call.Name,
        ...
        catch (Exception ex) when (_exceptionPolicy == ToolExceptionPolicy.ReturnErrorToModel)
        {
            ...
            var nested = NestedOf(toolScope); // sub-calls may have completed before the throw
            return (new ToolInvocation
            {
                Iteration = iteration,
                Name = call.Name,
        ...
        {
            // Pop on every path — success, timeout, ReturnErrorToModel, and the
            // uncaught ThrowToCaller case (where the exception propagates out).
            _tracker.EndScope(toolScope);
        }
    }

    /// <summary>
    /// Summarizes the AI calls made inside a tool's grouping node. Returns nulls when
    /// the tool made no AI calls (the common case for non-AI tools).
    /// </summary>
    private static (TokenUsage? Usage, Price? Cost, int? Calls) NestedOf(AiUsageScopeBuilder toolScope)
    {
        var frozen = toolScope.Freeze();
        var calls = frozen.ModelCalls();
        return calls.Count == 0
            ? (null, null, null)
            : (frozen.TotalUsage, frozen.TotalCost, calls.Count);
    }
