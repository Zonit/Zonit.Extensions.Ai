...
public static class AiServiceCollectionExtensions
{
        ...
        Action<AiOptions>? options = null)
    {
        ...
        // Nested AI usage tracker. Singleton with no shared mutable state — the
        // "current node" lives in an AsyncLocal, so concurrent requests are isolated
        // by their async flow. Exposed read-only via IAiUsageTracker; the framework
        // (AgentRunner/ToolExecutor/AiProvider) drives it through the concrete type.
        services.TryAddSingleton<AiUsageTracker>();
        services.TryAddSingleton<IAiUsageTracker>(sp => sp.GetRequiredService<AiUsageTracker>());
        ...
        // Auto-discovery of ToolBase<,> types (via ToolDiscovery, populated by
        // a [ModuleInitializer] the source generator emits in every consumer
        // assembly) registers each tool as its CONCRETE type only — the type
        // becomes resolvable via `sp.GetRequiredService<MyTool>()` so callers
        // can pass it per-invocation as `tools: [sp.GetRequiredService<MyTool>()]`.
        //
        // It does NOT enroll the type into the global `IEnumerable<ITool>`
        // bucket consumed by IToolRegistry — that bucket is the "active for
        // every agent call" channel and must remain explicit opt-in via
        // AddAiTools<T>() / AddAiTools(instance). Auto-enrolling discovered
        // tools turned every ToolBase<,> in any loaded assembly into an
        // implicit default for every agent invocation, leaking unintended
        // capabilities (and cost) into unrelated call sites.
