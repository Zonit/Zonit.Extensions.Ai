...
/// <summary>
/// LIVE end-to-end smoke test of the OpenAI agent tool-calling loop against the real
/// Responses API. Opt-in only — spends real credits, so it stays inert in CI: set
/// <c>ZONIT_OPENAI_SMOKE=1</c> and <c>ZONIT_OPENAI_KEY=&lt;sk-...&gt;</c> to enable.
/// </summary>
public class OpenAiAgentLiveSmokeTests
{
    private readonly ITestOutputHelper _output;
    public OpenAiAgentLiveSmokeTests(ITestOutputHelper output) => _output = output;

    private IAiProvider? Build()
    {
        if (Environment.GetEnvironmentVariable("ZONIT_OPENAI_SMOKE") != "1") return null;
        var key = Environment.GetEnvironmentVariable("ZONIT_OPENAI_KEY");
        if (string.IsNullOrWhiteSpace(key)) { _output.WriteLine("ZONIT_OPENAI_KEY not set — skipping."); return null; }
    ...
    // SCENARIO 1: plain string agent + tool.
    [Fact]
    public async Task Agent_OnOpenAi_CallsCSharpTool_AndUsesItsResult()
    {
        var ai = Build();
    ...
    // SCENARIO 2: structured-output agent + tool. The first request sends text.format;
    // continuation requests chain via previous_response_id. If the final (post-tool)
    // answer is no longer schema-constrained, parsing into the typed result fails.
    [Fact]
    public async Task Agent_OnOpenAi_WithStructuredOutput_StillCallsTool()
    {
        var ai = Build();
        if (ai is null) return;

        var result = await ai
            .Agent(new GPT4o(), new CodeAnswerPrompt
            {
    ...
    // SCENARIO 3: two sequential tool calls (3+ turn loop over previous_response_id).
    [Fact]
    public async Task Agent_OnOpenAi_TwoSequentialToolCalls_CompleteLoop()
    {
        var ai = Build();
        if (ai is null) return;
        ...
        _output.WriteLine($"Iterations: {result.Iterations}, ToolCalls: {result.ToolCalls.Count}, Final: {result.Value}");
        // GPT-4o may compute one of the two steps itself, so we don't demand exactly 2
        // tool calls — only that the continuation-over-previous_response_id loop ran a
        // tool round-trip and produced the right answer.
        result.ToolCalls.Should().NotBeEmpty();
        result.Value.Should().Contain("50");
    }

    // SCENARIO 4: raw ITool with a non-strict schema (MCP-like).
    [Fact]
    public async Task Agent_OnOpenAi_WithNonStrictToolSchema_DoesNotError()
    {
        var ai = Build();
        if (ai is null) return;
